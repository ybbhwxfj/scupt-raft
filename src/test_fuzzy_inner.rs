...
pub mod tests {
    ...
    use scupt_fuzzy::{event_setup, fuzzy_test_setup, fuzzy_test_unset};
    ...
    use scupt_fuzzy::initializer::{Initializer, InitializerPhantom};
    use scupt_util::serde_json_string::SerdeJsonString;
    use sedeve_kit::action::action_serde_json_string::ActionSerdeJsonString;
    use sedeve_kit::action::action_type::ActionType;
    ...
    struct TestRaftFuzzy<M:MsgTrait + 'static> {
        ...
        init_message: HashMap<NID, Message<SerdeJsonString>>,
    ...
    struct StateInit {
        map : HashMap<NID, Message<SerdeJsonString>>,
    }
    impl Initializer for StateInit {
        fn message(&self) -> Vec<Message<SerdeJsonString>> {
            self.map.values().map(|x|{ x.clone() }).collect()
        }
    }
    impl EventCtrl {
        fn new() -> Self {
    ...
    #[derive(Clone)]
    struct Generator<T:MsgTrait + 'static> {
        crash_ratio:f64,
    ...
    impl <T:MsgTrait> Generator<T> {
        fn new(vec:Vec<NID>) -> Self {
            ...
            Self {
                ...
                _phantom: Arc::new(Default::default()),
            }
        }
    ...
    impl <T:MsgTrait + 'static> FuzzyGenerator for Generator<T> {
         fn gen(&self, cmd: FuzzyCommand) -> Vec<FuzzyEvent> {
            return match cmd {
                FuzzyCommand::MessageReq(m) => {
                    let mut v0 = vec![];
                    let mut v1 = self.gen_message_fuzzy(m.clone());
                    v0.append(&mut v1);
    ...
    fn read_initialize_state_setup_trace(trace:String) -> Res<HashMap<NID, Message<SerdeJsonString>>> {
        let incoming = ActionIncomingFactory::action_incoming_from_string(trace)?;
        let mut map = HashMap::new();
        loop {
            let r = incoming.next();
            match r {
                Ok(s) => {
                    let serde_string = ActionSerdeJsonString::from_json_string(s);
                    let serde_value = serde_string.to_action_serde_json_value()?;
                    let source = serde_value.source_node_id()?;
                    let dest = serde_value.dest_node_id()?;
                    let action_type = serde_value.action_type()?;
                    assert_eq!(action_type, ActionType::Setup);
                    let msg_string = serde_json::to_string(&serde_value.message_payload_json_value()?).unwrap();
                    let m = Message::new(SerdeJsonString::new(msg_string), source, dest);
                    map.insert(m.dest(), m);
                }
                Err(e) => {
                    if e == ET::EOF {
    ...
    impl <M:MsgTrait + 'static> TestRaftFuzzy<M> {
        ...
        fn run_fuzzy_server_inner(
            ...
            let initializer : Arc<dyn Initializer> = if init_message.is_empty() {
                Arc::new(InitializerPhantom::default())
            } else {
                Arc::new(StateInit {
                    map:init_message
                })
            ...
            let server = FuzzyServer::new(
                ...
                notifier, server_addr, peers,
                Arc::new(Generator::<M>::new(node_id)), initializer)?;
    ...
    pub fn run_raft_fuzzy(
        inject_events:Option<String>,
        opt_trace:Option<String>,
        nodes:u32,
        seconds_max:u64
    ) -> Res<()> {
        ...
        let map = match opt_trace {
            None => {
                HashMap::new()
            }
            Some(s) => {
                read_initialize_state_setup_trace(s)?
            }
        };
        ...
        fuzzy_test_setup!(RAFT_FUZZY, test.fuzzy_client_connect_to_server.node_id, addr.to_string());
